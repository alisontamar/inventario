import {
  ...
  TextInput, Image, ScrollView, Alert, Platform
} from "react-native";
...
const subirImagen = async (uri: string): Promise<string | null> => {
  try {
    ...
    console.log('Iniciando subida de imagen con URI:', uri);

    const fileName = `producto_${Date.now()}.jpg`;

    // Leer archivo y convertirlo a blob
      ...
      {step==="record" && (
        <View style={styles.centered}>
          <Text style={styles.title}>
            {isListening ? "Escuchando..." : "Información del Producto"}
          </Text>
          ...
          {type==="inventory"&&(
            <>
              <TouchableOpacity style={styles.photoButton} onPress={pickImage}>
                <Text style={styles.optionText}>📷 Tomar foto del producto</Text>
              </TouchableOpacity>
